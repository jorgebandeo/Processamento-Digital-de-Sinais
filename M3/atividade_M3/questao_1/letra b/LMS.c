...
int main() {
    ...
    double u = 0.00000000001; // Passo de adaptação escolhido
    int K = 100; // Tamanho do filtro

    // Abrir arquivos de entrada
    file_x = fopen("ruido_branco.pcm", "rb");
    file_d = fopen("dn_Q1.pcm", "rb");
    file_out_erro = fopen("erro_Q1_C.pcm", "wb");
    file_out_y = fopen("saida_Q1_C.pcm", "wb");
    ...
    printf("Sinal de erro salvo em 'erro_Q1_C.pcm'.\n");
    printf("Sinal de saída salvo em 'saida_Q1_C.pcm'.\n");
